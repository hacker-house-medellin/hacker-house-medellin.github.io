...
async function initialize(activeForm) {
  const apiOrigin = exactHttpsOrigin(document.documentElement.dataset.apiOrigin);
  const supabaseOrigin = exactHttpsOrigin(document.documentElement.dataset.supabaseOrigin);
  const siteKey = document.documentElement.dataset.turnstileSiteKey?.trim() ?? '';
...
async function submitApplication(activeForm, apiOrigin, supabaseOrigin, proofs) {
  ...
  const resumeUploadId = await uploadFile({
    apiOrigin,
    supabaseOrigin,
    proofs,
    nonce,
  ...
  const photoIdUploadId = await uploadFile({
    apiOrigin,
    supabaseOrigin,
    proofs,
    nonce,
...
async function uploadFile({ apiOrigin, supabaseOrigin, proofs, nonce, kind, file }) {
  ...
  try {
    ...
    // The upload outcome can be ambiguous. Completion is authoritative because
    // the API streams and verifies the private object before accepting it.
  }
